...
const Menu = () => {
  ...
  const contentItems = [
    ...
    { icon: FaHdd, label: "Backups", path: "/backups" },
  ];
  const analyticsItems = [
    { icon: FaChartBar, label: "Analytics", path: "/analytics" },
    { icon: FaClipboard, label: "All Logs", path: "/logs" },
  ];
  ...
  const MenuItem = ({ icon: Icon, label, path }) => (
    <li className="group">
      <Link
        to={path}
        className="flex items-center space-x-4 px-5 py-4 rounded-2xl cursor-pointer transition-all duration-300 hover:scale-105 hover:bg-white/10"
      >
  ...
  const MenuSection = ({ title, items }) => (
    <div className="mb-10">
      <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-4 px-5">
        {title}
      </h3>
